def checkPasswordPolicy(list):
    cntr = 0
    for policy in list:
        dash_pos = policy.index("-")
        min = int(policy[0:dash_pos])

        first_space = policy.index(" ")
        max = int(policy[dash_pos + 1:first_space])

        colon_pos = policy.index(":")
        letter = policy[colon_pos - 1]

        password = policy[colon_pos + 2:]
        count = password.count(letter)
        if count >= min and count <= max:
            cntr += 1
    return cntr

def checkPasswordPolicyPos(list):
    cntr = 0
    for policy in list:
        dash_pos = policy.index("-")
        first_pos = int(policy[0:dash_pos]) - 1

        first_space = policy.index(" ")
        second_pos = int(policy[dash_pos + 1:first_space]) - 1
        ...
        if first_pos <= len(password):
            if password[first_pos] == letter:
                flag += 1

        if second_pos <= len(password):
            if password[second_pos] == letter:
                flag += 1
...
if __name__ == '__main__':
    list = """13-17 s: ssssssssssssgsssj
7-9 p: pnlzhcppvl
5-6 z: zzbwrv
...
4-5 n: ngtnr
3-12 k: mtcfszkdhkdkd""".split("\n")

print("Length of passwords: " + str(len(list)))
#print("Valid passwords: " + str(checkPasswordPolicy(list)))
print("Valid passwords: " + str(checkPasswordPolicyPos(list)))
